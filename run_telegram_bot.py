...
async def main():
    ...
    try:
        ...
        # Mantener el bot corriendo de forma asíncrona
        while True:
        ...
        if telegram_handler:
            # Aseguramos que el método stop sea llamado si existe
            try:
                await telegram_handler.stop()
            except Exception as stop_error:
                logger.error(f"Error al cerrar el handler: {stop_error}")
...
if __name__ == "__main__":
    # Registrar manejador de señales para cierres limpios
    signal.signal(signal.SIGINT, signal_handler)
    signal.signal(signal.SIGTERM, signal_handler)
    
    # Ejecutar bot usando el loop de asyncio
    try:
        asyncio.run(main())
    except KeyboardInterrupt:
