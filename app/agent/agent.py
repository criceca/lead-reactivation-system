...
class SimpleLeadReactivationAgent:
    ...
    def initiate_reactivation(self, lead_id: int, conversation_id: int) -> Dict[str, Any]:
        ...
        try:
                ...
                return {"success": False,"error": str(e),"message": "Lo siento, hubo un error al iniciar la conversación."}
    ...
    def process_message(self, conversation_id: int,lead_message: str) -> Dict[str, Any]:
        ...
        try:
            ...
            crud.create_message( db=self.db,conversation_id=conversation_id,role="lead",content=lead_message )

            # Cargar historial
            chat_history = self.load_conversation_memory(conversation_id)

            # Crear mensajes
            messages = [SystemMessage(content=SYSTEM_PROMPT), *chat_history, HumanMessage(content=lead_message)]
            ...
            return {"success": True,"message": response_text,"tools_used": [],"escalated": escalated,"requirements_captured": requirements_captured,}

        except Exception as e:
            logger.error(f" Error processing message: {e}")
            return {"success": False,"error": str(e),"message": "Lo siento, hubo un error procesando tu mensaje." }
    ...
    def get_conversation_summary(self, conversation_id: int) -> Dict[str, Any]:
         ...
         try:
            ...
            summary_prompt = f"""Analiza esta conversación y proporciona un resumen en JSON:{conversation_text}
                                 Formato JSON:{{"resumen_general": "Resumen breve",
    ...
    def get_agent_info(self) -> Dict[str, Any]:
        ...
        return {"provider": model_info["provider"],
