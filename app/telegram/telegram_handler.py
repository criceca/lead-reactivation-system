...
class ImprovedTelegramHandler:
 ...
 # Buscar por username de Telegram (si el lead tiene @username en el teléfono)
 if user.username:
 logger.info(f" Buscando lead con username: @{user.username}")
 # Buscar leads que tengan @username en el campo phone
 ...
 # Si no se encontró, buscar por telegram_user_id en email temporal
 if not lead:
 ...
 # Si el lead existe y está frío, cambiar a warm
 if lead.status == "cold":
 ...
 # PASO 4: Mensaje de bienvenida personalizado
 if lead.status == "cold":
...
/escalate - Solicitar hablar con un humano"""
 ...
 # PASO 5: Iniciar agente con contexto del lead
 ...
 logger.info(" Enviando respuesta del agente...")
 await update.message.reply_text(result["message"])
 logger.info(" Respuesta del agente enviada")
 else:
...
Simplemente escribe tu mensaje y yo te responderé."""
 ...
 # Obtener conversación del contexto
 conversation_id = context.user_data.get('conversation_id')
 lead_id = context.user_data.get('lead_id')
 ...
 try:
 # Verificar que la conversación no esté escalada
 ...
 # Si se escaló automáticamente, notificar
 ...
 Args:
 lead_id: ID del lead
 message: Mensaje inicial a enviar
 ...
 # Verificar que el lead tenga teléfono de Telegram
 if not lead.phone or "@" not in lead.phone:
 logger.error(f"Lead {lead_id} no tiene username de Telegram en el campo phone")
 return False
 
 # Extraer username (quitar @ si existe)
 username = lead.phone.replace("@", "").strip()
 
 # Buscar conversaciones existentes del lead
 ...
 logger.warning(
 f"Lead {lead_id} (@{username}) no ha iniciado conversación aún. "
 f"El usuario debe enviar /start primero al bot."
 )
 return False
 ...
 if success:
 logger.info(f" Contacto iniciado con lead {lead_id} (@{username})")
 
 # Actualizar lead a warm si está cold
 if lead.status == "cold":
